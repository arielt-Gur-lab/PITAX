...
stage2_initialize_read_assignment <- function(file_name, default_locus = "Unassigned", default_direction = "Unknown", forward_primer = "", reverse_primer = "") {
...
stage2_make_read_assignments <- function(files, default_locus = "Unassigned", default_direction = "Unknown", forward_primer = "", reverse_primer = "") {
...
stage2_empty_assignments <- function() {
  data.frame(
    ...
    Final_Name = character(), Isolate = character(), Locus = character(), Direction = character(),
...
stage2_sync_generated_names <- function(assignments, forward_primer = "", reverse_primer = "") {
  ...
  for (i in seq_len(nrow(assignments))) {
    ...
    if (assignments$Direction[i] == "Forward" && nzchar(stage2_scalar_text(forward_primer))) assignments$Primer[i] <- stage2_scalar_text(forward_primer)
    if (assignments$Direction[i] == "Reverse" && nzchar(stage2_scalar_text(reverse_primer))) assignments$Primer[i] <- stage2_scalar_text(reverse_primer)
...
stage2_validate_assignments <- function(df, expected_source_ids = NULL) {
...
stage2_build_architecture <- function(assignments, project_id = "project") {
  assignments <- stage2_coerce_assignments(assignments)
  error <- stage2_validate_assignments(assignments)
  ...
  reads <- assignments[, c("Read_ID", "Locus_ID", "Isolate_ID", "Source_ID", "File", "Final_Name", "Direction", "Primer", "Inference", "Notes"), drop = FALSE]
  list(
    schema = "pitax-project-architecture-v1",
