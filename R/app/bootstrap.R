...
pitax_source(file.path("R", "domain", "assignment", "stage2_architecture.R"), local = TRUE)
pitax_source(file.path("R", "domain", "consensus", "stage3_consensus.R"), local = TRUE)
pitax_source(file.path("R", "domain", "multilocus", "stage4_multilocus.R"), local = TRUE)
pitax_source(file.path("R", "domain", "sanger", "core_sanger.R"), local = TRUE)
...
APP_VERSION <- tryCatch(trimws(readLines("VERSION.txt", warn = FALSE)[1]), error = function(e) "3.0.0-alpha.10.2")
APP_VERSION <- ifelse(is.na(APP_VERSION) || !nzchar(APP_VERSION), "3.0.0-alpha.10.2", APP_VERSION)
