  ...
  reset_pipeline_state <- function() {
    ...
    rv$assay_profiles <- assay_default_profiles(); rv$project_defaults <- assay_project_defaults_from_legacy_settings()
    rv$read_assignments <- stage2_empty_assignments(); rv$architecture <- NULL; rv$consensus_set <- stage3_empty_consensus_set(); rv$multilocus_profile <- stage4_empty_profile(); rv$multilocus_imports <- list(); rv$project_migration_log <- ""
