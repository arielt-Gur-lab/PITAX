  ...
  output$taxonomy_counts_table <- renderDT({
    ...
    friendly <- c(
      ...
      delta_identity_pp="DeltaIdentity (pp)",
      delta_coverage_pp="DeltaCoverage (pp)",
  ...
  output$taxonomy_hits_table <- renderDT({
    ...
    defs <- list(
      ...
      list(targets=unlist(lapply(c("Identity (%)","Coverage (%)","E-value","Bit score","DeltaBit"), idx)), width="68px", className="dt-compact-number"),
