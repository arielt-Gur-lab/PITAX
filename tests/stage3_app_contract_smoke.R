# PITAX v3.0.0-alpha.8.2 - Stage 3 Shiny integration and startup tests.

read_text <- function(path) paste(readLines(path, warn = FALSE, encoding = "UTF-8"), collapse = "\n")
app_text <- read_text("app.R")
consensus_text <- read_text(file.path("R", "stage3_consensus.R"))
export_text <- read_text(file.path("R", "export_tools.R"))
...
must_contain(app_text, 'source(file.path("R", "stage3_consensus.R"), local = TRUE)')
...
must_contain(app_text, 'autoWidth = FALSE')
...
fixture_path <- normalizePath(file.path("TEST", "Stage3_synthetic_pair", "TESTPAIR001_forward.ab1"), winslash = "/", mustWork = TRUE)
...
cat("v3.0.0-alpha.8.2 Stage 3 app integration and startup tests passed.\n")
